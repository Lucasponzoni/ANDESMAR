...
function mostrarResultados(resultados) {
    ...
    // Ordenar no en arrays
    noEnArraysResult.sort((a, b) => {
        const skuA = a.match(/>([^<]+)</)[1]; // Extraer SKU
        const skuB = b.match(/>([^<]+)</)[1];
        
        if (/^\d/.test(skuA) && !/^\d/.test(skuB) ) return -1; // Números primero
        if (!/^\d/.test(skuA) && /^\d/.test(skuB) ) return 1; // Letras después
        return skuA.localeCompare(skuB); // Ordenar alfabéticamente
    });
...
btnImprimir.onclick = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            setTimeout(() => {
                generarPDF(tabla); // Llama a la función para generar PDF
                marcarComoPreparado(resultados); // Marca como preparado en Firebase
            }, 3000); // Esperar 3 segundos antes de generar el PDF
        }
    });
};

tablaContainer.insertBefore(btnImprimir, tabla); // Coloca el botón encima de la tabla


function marcarComoPreparado(resultados) {
    const dbRef = database.ref('envios');
    const promises = []; // Array para almacenar las promesas de actualización

    for (const sku in resultados) {
        const item = resultados[sku]; // Obtener el objeto correspondiente
        const cantidad = item.cantidad; // Obtener la cantidad total
        const idOperacion = item.idOperacion; // Obtener idOperacion

        console.log(`SKU: ${sku}, idOperacion: ${idOperacion}, Cantidad: ${cantidad}`); // Verificar el SKU y la cantidad

        if (idOperacion) { // Verificar que idOperacion existe
            // Buscar todos los nodos que coinciden con el SKU
            const queryRef = dbRef.orderByChild('SKU').equalTo(sku);
            const updatePromise = queryRef.once('value').then((snapshot) => {
                let updatedCount = 0; // Contador de nodos actualizados

                snapshot.forEach((childSnapshot) => {
                    const childKey = childSnapshot.key; // Obtener la clave del nodo

                    // Actualiza el nodo
                    const updateNode = dbRef.child(childKey).update({ query: "preparado" });
                    promises.push(updateNode); // Agregar la promesa de actualización al array
                    updatedCount++; // Incrementa el contador
                });

                console.log(`Actualizados ${updatedCount} nodos para SKU: ${sku}`);
            });

            promises.push(updatePromise); // Agregar la promesa de consulta al array
        } else {
            console.warn(`No se encontró idOperacion para SKU: ${sku}`);
        }
    }
    ...
    Promise.all(promises)
        .then(() => {
            console.log("Todos los productos han sido marcados como preparados.");
        })
        .catch((error) => {
...
function generarPDF(tabla) {
    ...
    rows.forEach((row) => {
        ...
        const sku = cells[0].innerText;
        const cantidad = cells[1].innerText;
        const producto = cells[2].innerText;
